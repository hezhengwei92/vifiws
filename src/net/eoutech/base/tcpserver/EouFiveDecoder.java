 ...
 */
public class EouFiveDecoder extends ByteToMessageDecoder {
    ...
    protected void decode(ChannelHandlerContext ctx, ByteBuf in, List<Object> list) throws Exception {
        LogUtils.info("进入SERVER FIVE DECODER，START" + " 所用通道" + ctx.channel());
                    ...
                    if (size > 14) {//长度不够，无需处理
                        if (cha[0] == (char) buffer.get(count)) {//处理ByteBuf中的数据，或者ByteBuffer中有数据需要处理
                            if ((cha[1] == (char) buffer.get(count + 1) && cha[2] == (char) buffer.get(count + 2) && cha[3] == (char) buffer.get(count + 3))) {
                                ...
                                FramesData frameData = getEouData(ctx, buffer);
                                if (frameData.getDoOrNot()) {//完整数据再处理
                                    ...
                                    byte msg = byteBuffer.get(4);

                                    for (int i = 0; i < 5; i++) {//帧头+通信主体
                                        bytes[i] = byteBuffer.get();
                                    }

                                    byte key = byteBuffer.get(lengths - 5);//获取用来加解密的关键值

                                    for (int i = 5; i < lengths - 6; i++) {
                                        byte temp = byteBuffer.get();//获取解密后的值
                                        if ((temp & 0xFF) == 0xFA && ((msg >> 4) & 0x0F) == 0x01) {//处理特殊字符
                                            bytes[i] = (byte) ~(byteBuffer.get() ^ key);
                                            lengths--;//特殊字符处理，长度需要减掉1
                                            move++;
                                        } else {
                                            bytes[i] = (byte) (temp ^ key);
                                        }
                                    }

                                    for (int i = lengths - 6; i < lengths; i++) {//CRC+帧尾
                                        bytes[i] = byteBuffer.get();
                                    }
...
//                    FlumeRpcClientUtils.appendBatch(arrList);
    ...
    private static FramesData getEouData(ChannelHandlerContext ctx, ByteBuffer byteBuffer) {
        int length = 0;
        boolean flag = false;
